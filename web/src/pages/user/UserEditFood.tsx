...
const getEditExpiryMeta = (savedAt?: string | number) => {
  ...
  if (!savedAtMs) {
    return {
      isExpired: false,
      remainingDays: EDIT_WINDOW_DAYS,
      message: `⏳ ${EDIT_WINDOW_DAYS} days left to edit`,
    };
  }
  ...
  return {
    ...
    message: isExpired
      ? "❌ Edit expired"
      : `⏳ ${remainingDays} day${remainingDays > 1 ? "s" : ""} left to edit`,
  };
};
...
const parseTimeParts = (value?: string) => {
  const [timePart = "", meridiem = "AM"] = (value ?? "").split(" ");
  const [h = "12", m = "00"] = timePart.split(":");
  return { hour: h, minute: m, ampm: meridiem };
};
...
const toMenuLines = (menu: MenuCategory[]) => {
  ...
  const lines = safeMenu
    .map((category) => {
      ...
      return `${category.category || "Category"}: ${cleanItems.join(", ")}`;
    })
    .filter(Boolean);
...
const buildOriginalSnapshot = (editData: EditData): BookingSnapshot => ({
  session: editData.session ?? "",
  time: editData.time ?? "",
  foodType: editData.foodType ?? "",
  menu: editData.menu ?? [],
  vegetables: editData.vegetables ?? [],
  vendors: (editData.vendors ?? []).map(normaliseVendor),
});
...
const snapshotsDiffer = (before: BookingSnapshot, after: BookingSnapshot) => {
  return (
    before.session !== after.session ||
    before.time !== after.time ||
    before.foodType !== after.foodType ||
    !sameLines(toMenuLines(before.menu), toMenuLines(after.menu)) ||
    !sameLines(toVegetableLines(before.vegetables), toVegetableLines(after.vegetables)) ||
    !sameLines(toVendorLines(before.vendors), toVendorLines(after.vendors))
  );
};
...
const SectionCard = ({ children, className = "" }: { children: ReactNode; className?: string }) => (
  <div className={`bg-white border border-green-100 rounded-2xl shadow-sm ${className}`}>{children}</div>
);
...
const UserEditFood = () => {
  ...
  useEffect(() => {
    ...
    const { hour: h, minute: m, ampm: meridiem } = parseTimeParts(editData.time);
    setHour(h);
    setMinute(m);
    setAmpm(meridiem);
  ...
  const handleSave = () => {
    if (saveState !== "idle" || !editData || editStatus.isExpired || !id) return;

    const before = buildOriginalSnapshot(editData);
    const draft = buildDraftSnapshot(editData, session, displayTime, foodType);

    if (!snapshotsDiffer(before, draft)) {
      navigate(`/book-food/${id}/dashboard`);
      return;
    }
    ...
    window.setTimeout(() => {
      ...
      const updated = stored.map((entry) =>
        entry.eventId === id && entry.version === editData.version
          ? {
              ...
              approvalOriginal: {
                session: entry.session ?? editData.session ?? "",
                time: entry.time ?? editData.time ?? "",
                foodType: entry.foodType ?? editData.foodType ?? "",
                menu: entry.menu ?? editData.menu ?? [],
                vegetables: entry.vegetables ?? editData.vegetables ?? [],
                vendors: entry.vendors ?? (editData.vendors ?? []).map(normaliseVendor),
              },
              pendingUpdate: {
                session,
                time: displayTime,
      ...
      setSaveState("saved");
      window.setTimeout(() => navigate(`/book-food/${id}/dashboard`), 1000);
    }, 700);
  };
  ...
  const handleTouchEnd = (e: TouchEvent<HTMLDivElement>) => {
    if (e.changedTouches[0].clientX - startX.current > 100) goBack();
  };
    ...
    return (
      <div className="min-h-screen bg-[#f4faf6] flex items-center justify-center px-4">
        <div className="text-center bg-white rounded-2xl p-8 shadow-md max-w-sm w-full border border-green-100">
          ...
          <button onClick={goBack} className="bg-green-700 text-white font-bold px-5 py-2.5 rounded-xl hover:bg-green-800 transition w-full">
            Go Back
          </button>
        </div>
      </div>
  ...
  const CountdownCard = (
    <div className={`rounded-2xl border overflow-hidden ${cardBorder}`}>
      <div className="px-5 pt-5 pb-4 flex items-start gap-3 border-b border-white/60">
        <span className="text-xl flex-shrink-0 mt-0.5">{tick.isExpired ? "🔒" : isUrgent ? "⚡" : "✏️"}</span>
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-1">
            <p className={`text-xs font-extrabold tracking-wide ${accentText}`}>Editing · List {editData.version}</p>
          ...
          <p className={`text-[11px] leading-relaxed ${subText}`}>Changes will be sent to admin for approval.</p>
        </div>
      </div>
          ...
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl px-4 py-5 border border-white/90 shadow-sm">
            ...
            {isLastSecs && (
              <p className="text-center text-[10px] font-bold text-red-500 animate-pulse mt-3">
                ⚠ Submit now — expiring in seconds!
              </p>
            )}
            {isUrgent && !isLastSecs && (
              <p className="text-center text-[10px] font-bold text-orange-500 mt-3">⚡ Less than 6 hours remaining</p>
            )}
          </div>
        ) : (
      ...
      <div className="px-5 pb-5">
        <div className="bg-white/50 rounded-xl px-4 py-4 border border-white/70">
          ...
          <TimelineBars remainingDays={tick.remainingDays} progressPct={tick.progressPct} isExpired={tick.isExpired} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#f4faf6] font-sans" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      <button onClick={handleSave} disabled={saveState !== "idle" || editStatus.isExpired} className={saveBtnCls}>
        {saveBtnContent()}
      </button>
      ...
      <div className="sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-green-100 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-14 flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 text-xs font-medium text-gray-400 flex-1 min-w-0">
            <span className="hover:text-gray-600 cursor-pointer transition-colors" onClick={() => navigate(`/book-food/${id}/dashboard`)}>
              Dashboard
            </span>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 pb-28 sm:pb-10">
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="w-full lg:w-[300px] flex-shrink-0 lg:sticky lg:top-[72px] h-fit">{CountdownCard}</div>
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["Morning", "Afternoon", "Evening", "Night"].map((s) => (
                      <button key={s} onClick={() => setSession(s)} className={sessionPill(s)}>
                        {s}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["Veg", "Non Veg", "Both"].map((t) => (
                      <button key={t} onClick={() => setFoodType(t)} className={foodTypePill(t)}>
                        {t}
                      </button>
                    ))}
                  </div>
            ...
            <FoodMenu eventName={eventName} location={location} session={session} displayTime={displayTime} />
