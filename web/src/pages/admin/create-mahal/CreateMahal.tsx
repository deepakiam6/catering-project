...
// ✅ REMOVED: Manager type — no longer needed in this file
...
const defaultImages = Array.from({ length: 10 }, (_, i) => `/images/mahal${i + 1}.jpg`);
...
export default function CreateMahal() {
  ...
  // ✅ REMOVED: managers state, selectedManagerPhone state
  ...
  const [mahal, setMahal] = useState<MahalType>({
    id: "", nameEnglish: "", fromDate: "", toDate: "",
    location: "", image: "", userId: "", password: "",
  });

  // ✅ Load event for editing (no manager pre-fill needed)
  useEffect(() => {
    if (!id) return;

    const storedEvents: MahalType[] = JSON.parse(localStorage.getItem("events") || "[]");
    const found = storedEvents.find((e) => e.id === id);
    if (found) {
      setMahal(found);
      setUserId(found.userId || "");
      setPassword(found.password || "");
      // ✅ REMOVED: setSelectedManagerPhone
    }
  }, [id]);

  // ✅ REMOVED: availableManagers computed list
  ...
  const handleImage = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => setMahal({ ...mahal, image: reader.result as string });
  ...
  const monthLabel = () => {
    ...
    if (!mahal.toDate || fm === tm) return `${tamilMonths[fm]} · ${englishMonths[fm]}`;
  ...
  const submit = () => {
    ...
    if (!userId.trim()) return setError("Phone number is required");
    if (!/^\d{10}$/.test(userId.trim())) return setError("Phone number must be 10 digits");

    const stored: MahalType[] = JSON.parse(localStorage.getItem("events") || "[]");

    const isDuplicate = stored.some(
      (e) => String(e.userId).trim() === String(userId).trim() && e.id !== id
    );
    if (isDuplicate) return setError("Phone number already exists");

    // ✅ Save event WITHOUT assignedManagerPhone
        ...
        <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-[22px] border border-emerald-100 bg-white p-7 shadow-[0_4px_24px_rgba(0,0,0,0.05)]">
            ...
            <div className={sectionTitleCls}>
              <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-green-600 text-[10px] text-white">1</span>
              <span>Event Details</span>
              <span className="h-px flex-1 bg-green-200" />
            </div>

            <div className="mb-4">
              <label className="mb-2 block text-[11px] font-bold uppercase tracking-[0.14em] text-green-700/70">Mahal Name</label>
              <input name="nameEnglish" value={mahal.nameEnglish} onChange={handleChange}
            ...
            <div className="mb-4">
              <label className="mb-2 block text-[11px] font-bold uppercase tracking-[0.14em] text-green-700/70">Location</label>
              <input name="location" value={mahal.location} onChange={handleChange}
            ...
            {/* Section 2: Dates */}
            <div className={sectionTitleCls}>
              <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-green-600 text-[10px] text-white">2</span>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="mb-4">
                <label className="mb-2 block text-[11px] font-bold uppercase tracking-[0.14em] text-green-700/70">From Date</label>
                <input type="date" name="fromDate" value={mahal.fromDate} onChange={handleChange}
              ...
              <div className="mb-4">
                ...
                <input type="date" name="toDate" value={mahal.toDate} onChange={handleChange}
                  className="w-full rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm font-medium text-green-950 outline-none transition focus:border-green-600 focus:bg-white focus:ring-4 focus:ring-green-600/10" />
              </div>
            </div>

            {/* Section 3: Credentials (manager assignment removed) */}
            <div className={sectionTitleCls}>
              <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-green-600 text-[10px] text-white">3</span>
              <span>Credentials</span>
              <span className="h-px flex-1 bg-green-200" />
            </div>

            <div className="mb-4">
              <label className="mb-2 block text-[11px] font-bold uppercase tracking-[0.14em] text-green-700/70">User Phone Number</label>
              <div className="relative flex items-center">
                <span className="pointer-events-none absolute left-3 text-gray-500"><PhoneIcon /></span>
            ...
            <div className="mb-4">
              <label className="mb-2 block text-[11px] font-bold uppercase tracking-[0.14em] text-green-700/70">Password</label>
              <div className="relative flex items-center">
                <span className="pointer-events-none absolute left-3 text-gray-500"><LockIcon /></span>
            ...
            {/* ✅ REMOVED: Assign Manager dropdown section entirely */}

            {/* Section 4: Cover Image */}
            <div className={sectionTitleCls}>
              <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-green-600 text-[10px] text-white">4</span>
            ...
            <label className="flex w-full cursor-pointer items-center gap-3 rounded-xl border border-dashed border-green-400 bg-green-50 px-4 py-3 text-green-600 transition hover:bg-green-100">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-green-200 to-green-300 text-sm">📷</div>
              <div>
                <div className="text-sm font-semibold">
                  {mahal.image?.startsWith("data:") ? "Image selected ✓" : "Upload from device"}
                </div>
                <div className="text-[11px] font-normal text-green-700/60">JPG, PNG, WEBP</div>
              </div>
              <input type="file" accept="image/*" onChange={handleImage} className="hidden" />
            </label>

            <p className="mb-2 mt-4 text-[11px] font-bold uppercase tracking-[0.14em] text-green-700/70">Quick Pick</p>
            <div className="grid grid-cols-5 gap-2">
              {defaultImages.map((img, i) => (
                <img key={i} src={img} alt="" onClick={() => setMahal({ ...mahal, image: img })}
                  className={`h-12 w-full cursor-pointer rounded-lg object-cover transition hover:scale-105 hover:shadow-md ${
                    mahal.image === img ? "scale-105 border-2 border-green-600 ring-4 ring-green-600/20" : "border-2 border-transparent"
                  }`} />
              ))}
            </div>
          ...
          <div className="rounded-[22px] border border-emerald-100 bg-white p-7 shadow-[0_4px_24px_rgba(0,0,0,0.05)]">
            ...
            <div className="relative h-[185px] overflow-hidden rounded-2xl">
              <img src={mahal.image || "/images/mahal1.jpg"} alt="Preview"
            ...
            <div className="mt-4">
              <p className={`text-lg font-extrabold ${!mahal.nameEnglish ? "italic font-normal text-green-300" : "text-green-950"}`}>
                {mahal.nameEnglish || "Mahal Name"}
              </p>
              <div className="my-3 h-px bg-gradient-to-r from-green-200 to-transparent" />
              <p className={`flex items-center gap-2 text-sm font-medium ${!mahal.location ? "italic text-green-300" : "text-green-700/80"}`}>
                <span>📍</span>
